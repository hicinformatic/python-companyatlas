...
from companyatlas.helpers import search_company_by_code
...
def _code_command(args: list[str]) -> bool:
    ...
    result = search_company_by_code(code=code, first=first, additional_args=additional_args, **providers_args)

    print(result)
    return True


code_command = Command(_code_command, "Search company by code (use --code company_code)")
